...
static void *run(hashpipe_thread_args_t * args)
{
    ...
    demo2_output_databuf_t *db_out = (demo2_output_databuf_t *)args->obuf;
	...
	hputi8(st.buf,"GPUMCNT",mcnt);
        ...
        while ((rv=demo2_input_databuf_wait_filled(db_in, curblock_in)) != HASHPIPE_OK) {
        ...
        while ((rv=demo2_output_databuf_wait_free(db_out, curblock_out)) != HASHPIPE_OK) {
            if (rv==HASHPIPE_TIMEOUT) {
                hashpipe_status_lock_safe(&st);
	...
	c=a+b; //using cpu in here, you can also using gpu if you have
	//sleep(0.1);
        db_out->block[curblock_out].sum=c;
	
        // Mark output block as full and advance
        demo2_output_databuf_set_filled(db_out, curblock_out);
        curblock_out = (curblock_out + 1) % db_out->header.n_block;

        // Mark input block as free and advance
        demo2_input_databuf_set_free(db_in, curblock_in);
...
static hashpipe_thread_desc_t demo2_gpu_thread = {
    name: "demo2_gpu_thread",
    skey: "GPUSTAT",
    init: NULL,
    run:  run,
    ibuf_desc: {demo2_input_databuf_create},
    obuf_desc: {demo2_output_databuf_create}
};

static __attribute__((constructor)) void ctor()
{
  register_hashpipe_thread(&demo2_gpu_thread);
}
